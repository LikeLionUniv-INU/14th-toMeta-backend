...
public class OpenAiCosmeticSearchClient {
    ...
    private static final String INSTRUCTIONS = """
            ...
            제품 상세 페이지 URL이 아닌 이미지 파일 URL을 반환한다.
            가능하면 공식 브랜드 사이트나 판매 페이지에서 확인 가능한 대표 이미지를 사용한다.
            실제 이미지 URL을 명확하게 확인할 수 없는 경우 빈 문자열을 반환한다.
            존재하지 않는 이미지 URL을 추측해서 만들지 않는다.

            mainIngredients:
            실제 해당 제품에서 확인할 수 있는 대표적인 주요 성분만 최대 5개 반환한다.
            전성분 전체를 반환하지 않는다.
            동일한 성분은 중복하지 않는다.
            확인할 수 없는 성분을 추측해서 만들지 않는다.

            제품의 주요 성분을 1개 이상 확인할 수 없는 경우 해당 제품은 결과에서 제외한다.
    ...
    public List<CosmeticSearchCandidate> search(String keyword) {
        try {
            ...
            log.debug("OpenAI 화장품 검색 output_text={}", outputText);

            SearchPayload payload = jsonMapper.readValue(
                    outputText,
                    SearchPayload.class
            );
            ...
            throw new GeneralException(
                    CosmeticErrorCode.COSMETIC_SEARCH_FAILED
            );
        }
    }
    ...
    private Map<String, Object> createResponseFormat() {
        ...
        itemProperties.put("productName", Map.of(
                "type", "string"
        ));
        ...
        itemProperties.put("imageUrl", Map.of(
                "type", "string"
        ));
    ...
    private void validateResponse(JsonNode response) {
        ...
        if (!"completed".equals(status)) {
            log.warn(
                    "OpenAI 화장품 검색 비정상 응답. status={}, response={}",
                    status,
                    response
            );
    ...
    private List<CosmeticSearchCandidate> normalize(
        ...
        Map<String, CosmeticSearchCandidate> uniqueItems =
                new LinkedHashMap<>();

        for (SearchItem item : items) {
            CosmeticSearchCandidate normalized =
                    normalizeItem(item);
    ...
    private CosmeticSearchCandidate normalizeItem(
            SearchItem item
    ) {
        if (item == null
        ...
        List<String> ingredients =
                normalizeIngredients(item.mainIngredients());
    ...
    private List<String> normalizeIngredients(
            List<String> ingredients
    ) {
        if (ingredients == null) {
            return List.of();
